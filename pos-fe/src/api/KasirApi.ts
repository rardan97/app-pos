...
export async function checkoutTransaction(token: string, data: CheckoutTransactionReq) : Promise<ApiResponse<CheckoutTransactionRes>>{
    ...
    try{
        const response = await api.post<ApiResponse<CheckoutTransactionRes>>(`${REST_API_BASE_URL}/checkout-transaction/create-checkout-transaction`, data, {
            headers: {
...
export async function checkoutTransactionStatus(token: string, data: TransactionReq) : Promise<ApiResponse<TransactionRes>>{
    ...
    try{
        const response = await api.post<ApiResponse<TransactionRes>>(`${REST_API_BASE_URL}/checkout-transaction/create-transaction-status`, data, {
            headers: {
                'Accept': 'application/json',
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
